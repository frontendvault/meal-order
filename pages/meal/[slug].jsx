  ...
  const renderMealTag = () => {
    return (
      <ul className="flex gap-2">
        {meal.tags.map((tag) => (
          <li className="bg-[#BFDBFE] rounded p-2 uppercase font-medium text-xs" key={tag}>{tag.name}</li>
        ))}
      </ul>
    );
  }
        ...
        <div className="p-4">
          {meal.nutrients.map((nutrient) => (
            <div class="flex items-center gap-4 mb-1">
              <div className="uppercase text-sm font-medium">{ nutrient.name }</div>
              ...
              <div className="flex gap-1 text-sm font-normal">
                ...
                <span>
                { `${nutrient.subValue ? `(${nutrient.subValue}${nutrient.subUnit ?? ''})` : ''}` }
                </span>
              </div>
            </div>
          ))}
